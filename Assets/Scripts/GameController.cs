...
public class GameController : MonoBehaviour 
{
	...
	public GameObject road, water_plane;
	...
	GameObject[] road_array, water_array;
	public float[] spawn_locations;
	//ArrayList obstacles;
	List<GameObject> obstacles;
	List<GameObject> obstacles_remove;

	public int fake_speed;
	public ParticleSystem EnergyPS;
	float speedConverter;
	int MAXSPEED;
	//int t
	public Text SpeedCounterObj;
	public CanvasGroup WinCanvas, UICanvas;
	...
	void Start () 
	{
        ...
        water_array = new GameObject[2];
        water_array[0] = Instantiate(water_plane, new Vector3(0.0f, 0.0f, 1f), Quaternion.identity) as GameObject;
        water_array[1] = Instantiate(water_plane, new Vector3(0.0f, 32.4f, 1f), Quaternion.identity) as GameObject;

        //= road_clone;
        MAXSPEED = 88;
		speedConverter = 88 / MAXSPEED;

		WinCanvas = GameObject.Find ("Win_Game").GetComponent<CanvasGroup> ();
		UICanvas = GameObject.Find ("UI").GetComponent<CanvasGroup> ();
		//fake_speed = 27;
		//textSpeed = 0;
		//SpeedCounterObj = gameObject.Find ("Speed").GetComponent<Speed_counter> ();
		//SpeedCounterObj = gameObject.GetComponent<Text>();
	...
	void Update () 
	{
        ...
        water_array[0].transform.Translate(movement * Time.deltaTime * (speed / 8f));
        water_array[1].transform.Translate(movement * Time.deltaTime * (speed/8f));

        if (water_array[0].transform.position.y < -32.5f)
        {
            Destroy(water_array[0].gameObject);
            water_array[0] = water_array[1];
            water_array[1] = Instantiate(water_plane, new Vector3(0, 31.8f,1f), Quaternion.identity) as GameObject;
        }
...
//		}
		...
		if (speed >= MAXSPEED) 
		{
			...
			EnergyPS.gameObject.SetActive (true);
			WinCanvas.alpha = 1;
		...
		{
			fake_speed = (int)(speed * speedConverter);
			speed += Time.deltaTime * 0.5f;
			//Debug.Log (fake_speed);

		}

		//SpeedCounterObj.speed_counter = fake_speed;
		SpeedCounterObj.text = fake_speed.ToString();
